  ...
  customBlock(node, context, convertors) {
    ...
    if (customConvertor) {
      try {
        ...
        console.warn(
          `[@licium/editor] - The error occurred when ${info} block node was parsed in markdown renderer: ${e}`
        );
      }
    }
  ...
  customInline(node, context, convertors) {
    ...
    if (customConvertor) {
      try {
        ...
        console.warn(
          `[@licium/editor] - The error occurred when ${nomalizedInfo} inline node was parsed in markdown renderer: ${e}`
        );
      }
    }
