...
describe('Bug #3308: Ordered list start numbering in Table', () => {
    ...
    it('should preserve start attribute for ordered list inside table', () => {
        ...
        const { table, tableRow, tableBody, tableHead, tableHeadCell, tableBodyCell, orderedList, listItem, paragraph, text } = schema.nodes;
        ...
        const doc = schema.nodes.doc.create({}, [
            table.create({}, [
                tableHead.create({}, [
                    tableRow.create({}, [
                        tableHeadCell.create({}, [
                            paragraph.create({}, [schema.text('Header')])
                        ])
                    ])
                ]),
                tableBody.create({}, [
                    tableRow.create({}, [
                        tableBodyCell.create({}, [
                            orderedList.create({ order: 3 }, [
                                listItem.create({}, [
                                    paragraph.create({}, [schema.text('Item 3')])
                                ])
                            ])
                        ])
                    ])
